...
class CourseAchievementSystem:
    ...
    def generate_visualizations(self, avg_achievements, output_dir='./visualizations'):
        ...
        plt.boxplot(student_achievements, tick_labels=['课程目标1', '课程目标2', '课程目标3', '课程目标4'])
    ...
    def generate_markdown_report(self, students, avg_achievements, weighted_achievement, output_file):
        """生成Markdown报告"""
        # 计算表现评价
        obj1_performance = "良好" if avg_achievements.get('课程目标1', 0) >= 0.7 else "一般"
        obj2_performance = "良好" if avg_achievements.get('课程目标2', 0) >= 0.7 else "一般"
        obj3_performance = "良好" if avg_achievements.get('课程目标3', 0) >= 0.7 else "一般"
        obj4_performance = "良好" if avg_achievements.get('课程目标4', 0) >= 0.7 else "一般"
...
**达成度：** {avg_achievements.get('课程目标1', 0):.4f}

从达成度结果可以看出，学生在掌握移动应用开发技术体系和主流平台特性方面表现{obj1_performance}。学生能够理解技术选型逻辑，熟悉跨平台开发框架和AI编程工具的基本使用。

#### 课程目标2分析
课程目标2主要考核学生运用跨平台开发框架及小程序技术，结合AI编程工具与后端API交互，设计实现跨平台应用，具备需求建模与创新应用能力。

**达成度：** {avg_achievements.get('课程目标2', 0):.4f}

从达成度结果可以看出，学生在运用跨平台开发框架及小程序技术方面表现{obj2_performance}。学生能够结合AI编程工具与后端API交互，设计实现跨平台应用。

#### 课程目标3分析
课程目标3主要考核学生调研对比多端开发方案，分析不同技术栈在跨设备适配场景中的优劣，具备技术方案评估与选型能力。

**达成度：** {avg_achievements.get('课程目标3', 0):.4f}

从达成度结果可以看出，学生在调研对比多端开发方案和技术方案评估方面表现{obj3_performance}。学生能够分析不同技术栈在跨设备适配场景中的优劣。

#### 课程目标4分析
课程目标4主要考核学生遵循软件工程规范，使用现代开发工具（含AI编程工具、Git版本控制）完成应用测试与优化，具备工程实践能力。

**达成度：** {avg_achievements.get('课程目标4', 0):.4f}

从达成度结果可以看出，学生在使用现代开发工具和工程实践方面表现{obj4_performance}。学生能够遵循软件工程规范，完成应用测试与优化。
